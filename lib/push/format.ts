...
export function buildNotification(event: PushEvent): PushPayload {
  switch (event.kind) {
    ...
    case "hw_due_moved": {
      ...
      // безличный глагол — пола в профиле нет, а «перенёс/перенесла» пришлось бы угадывать. «Перенесли на » плюс
      // самая длинная дата — «30 сентября 2026», 16 символов — это 29 символов, поэтому новая дата целиком
      // помещается в те примерно 40 символов, что видно в заголовке на экране блокировки; обрезаться может
      // только хвост названия предмета.
      const subject = event.subject?.trim();
      ...
      const moved = `Перенесли на ${dueDateLabel(event.dueDate, withYear)}`;
      return {
