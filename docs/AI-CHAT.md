...
Умолчания настроек (меняются админом): цена 200 ₽/мес, 15 сообщений в день, 45 в неделю, 3 сильных в неделю, триал 7 дней (сегодня — первый день). Худший случай при полном выборе лимитов ≈ 74,6 ₽ за 30 дней → маржа 63 %; при прежних 50/4 выходило 57 %, ниже требования. Инвариант «умолчания дают ≥ 60 %» закреплён тестом в `lib/assistant/estimate.test.ts`; админ видит ту же оценку под формой настроек.
