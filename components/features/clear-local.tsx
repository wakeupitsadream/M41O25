 ...
 *
 * Черновик и очередь домашки (raspison.hw.*) тоже не трогаем намеренно: сессия на iPhone протухает сама, и
 * стереть здесь — значит потерять несохранённое ДЗ у того же человека, который сейчас перезайдёт. Чужим оно не
 * покажется и не отправится: в записи лежит userId, и lib/hw/draft.ts стирает чужой черновик при первом же чтении.
 * Черновики помощника (raspison:assistant-draft:<userId>:…) живут по тому же правилу: чужие стирает экран чата.
 */
